...
### [Pranay → Akash] BUG: `alembic upgrade head` fails — `pledge_quotes` references `warehouses` before it exists
- **What happens:** a clean `docker compose up api` never reaches uvicorn. The migration raises `asyncpg.exceptions.UndefinedTableError: relation "warehouses" does not exist` while running `CREATE TABLE pledge_quotes (... FOREIGN KEY(warehouse_id) REFERENCES warehouses (id) ...)`. `warehouses` is created later in the ordering, so the FK has nothing to point at.
- **Why it matters:** it is the whole API, not one table — the container command is `alembic upgrade head && uvicorn ...`, so a failed migration means no server at all. Anyone cloning the repo fresh gets a backend that cannot start.
- **Workaround in place, and it is only a workaround:** the local compose runs `uvicorn` directly, skipping migrations, because the endpoints being exercised today (`/voice/transcribe`, `/voice/narrate`) are unauthenticated Sarvam proxies that touch no table. Every DB-backed route fails under that override; the app is on `USE_FIXTURES` for those. This is not a fix and should not be copied into the repo.
- **Suggested fix:** move the `warehouses` create ahead of `pledge_quotes` in the migration, or split the FK into a later `op.create_foreign_key`.
- **Raised:** H-current (2026-09-07)
