...
# MANDI-SETU - Build Order


### 6 people | 3 days | 6 lanes that do not collide


**Team:** Akash, Nikhil, Kartik, Nilesh, Pranay, Shreya. Six members. Shreya owns the farmer front-end (Lane 4).


**Window:** Friday to Sunday build. Pitch Monday to Tuesday.


**Target:** 80-90% for the college round. The rest before SIH.


> **The only rule that matters:** you may edit only the files your lane owns. Need a change elsewhere? Append to `docs/BLOCKERS.md`, stub locally, keep moving.
...
This is the document you hand to your teammates. It answers four questions and nothing else: where the project stands right now, what the six lanes are, what each lane does hour by hour, and how a teammate turns this page into a working Claude Code session.


It deliberately contains no code. The code lives in the repository, and the repository is the source of truth. This page is the index.


## The three-step onboarding, exactly


1. **Clone and boot the machine.** Five commands, listed in section 04. Ten minutes if the network behaves.
2. **Pick a lane** from the table in section 03. Six lanes, one each. Shreya takes Lane 4; the other five are open and you can trade among yourselves in the first ten minutes.
3. **Open Claude Code in the repo root and paste your lane's boot prompt** from section 06. It reads `CLAUDE.md` automatically, then reads your lane brief, then starts Task 1 and tells you its plan before writing files.


> **Why the boot prompt is a path, not a wall of text**  
...
## What is already written, and what this adds
...
This section is a verified inventory, not an estimate. Every line below was checked against the working tree on 4 September 2026.


## Committed and on disk
...
## Not written yet - and this is the honest part


> **There is no application code in this repository. None.**  
...
That is a better position than it sounds, and worse than it looks. Better, because the expensive decisions - the data model, the API surface, the demo numbers, the ownership boundaries - are made and frozen, which is exactly the work that six parallel agents cannot do concurrently. Worse, because every line of the product itself is still ahead of us and the first ninety minutes are single-threaded on one person.


## The single-threaded hour, named


Lane 1 must scaffold `apps/web` before Lanes 3, 4 and 6 have a directory to write into. Until that lands, three people cannot typecheck anything. Two mitigations, both in the calendar:
...
## Open items carried in from the last session
...
# 02. What "80-90% Complete" Means


Do not measure completeness in features, and do not measure it in commits. Measure it against this list. If all ten are demonstrably true on Monday morning, you are at 90% and the presentation writes itself. If eight are true and the two missing ones are named honestly on a roadmap slide, you are at 80% and still in a strong position.


| # | Statement that must be true on stage | Lanes |
|---|---|---|
...
> **C4, C5 and C9 are the three that win it.**  
...
Six lanes, six people, no shared files. The lane names match the role briefs already in `docs/roles/`, so your agent has a task list the moment you pick one. Lane 6 is new: it takes the buyer, FPO and admin consoles that were previously bundled into Lane 5.


| Lane | Codename | Owns | Suggested |
|---|---|---|---|
...
> **The suggested column is a suggestion. Trade in the first ten minutes, then stop.**  
...
## Why Lane 6 is consoles plus the deck
...
## Ownership map - the file boundaries, verbatim


This is the collision-prevention mechanism and it is the only one. Read your row. Do not edit outside it, not even a one-line fix, not even if it is obviously wrong.


| Lane | Paths you may create, edit and delete |
|---|---|
...
Anything not listed above belongs to Lane 1. Two paths deserve a specific warning because they look shared and are not: `apps/web/src/components/ui/**` is Lane 1's shadcn primitives, installed once - compose them, never modify them; and `apps/web/messages/{en,mr}.json` is Lane 4's, so if you need a string you request it rather than adding a key yourself.



---

# 04. Booting Your Machine and Your Agent


## Step 1 - the repository, once per person
...
Then, and only after Lane 1 has pushed the scaffold and the schema:
...
Lane 2 additionally, and the Python version is not a preference:
...
## Step 2 - your branch, once, and you stay on it


```BRANCH DISCIPLINE
git checkout -b l4-kisan      # substitute your own lane
...
> **Rebase, never merge. And an unpushed branch does not exist.**  
...
Open Claude Code in the repository root. It loads `CLAUDE.md` by itself. Paste the boot prompt for your lane from section 06. Every boot prompt has the same five parts, and the reason each part is there is worth knowing, because you will be re-pasting it at the start of every session for three days:


| Part | What it does | Why it is in there |
|---|---|---|
...
# 05. The Three-Day Calendar


> **The H-numbers in docs/ assume 72 hours. You have about 54, and roughly 36 of them are working hours per person.**  
...
Assuming a Friday 15:00 start. Shift every row by the same amount if you start later, and keep the ordering intact - the ordering is what the dependencies need.


| Brief says | Milestone | Do it by | What must be true |
|---|---|---|---|
...
## Day 1 - Friday. Foundations, and the scaffold is the bottleneck
...
## Day 2 - Saturday. The hero, and the chain
...
## Day 3 - Sunday. Harden, freeze, rehearse
...
# 06. The Six Lane Briefs and Their Boot Prompts


One page per lane. Read only your own. The boot prompt is what you paste into Claude Code at the start of every session - not just the first one, because a fresh session has no memory of the ownership rule and will cheerfully edit someone else's file without it.


## L1  SPINE  -  suggested: Pranay


**Mission.** Nobody else can move if the skeleton is not standing. Ship the scaffold, the schema, auth and the contracts before you ship anything clever.


**Order of work.** The scaffold in 90 minutes, then postgres, then auth end to end, then guard(), then deploy on day one.


**Your brief.** `docs/roles/R1_SPINE.md`


**Six things that decide whether this lane succeeds:**
...
```PASTE THIS INTO CLAUDE CODE, EVERY SESSION
I am L1 (codename SPINE) on MANDI-SETU, a Next.js 15 + Prisma + Postgres
...
Read these before you write anything, in this order:
  ...
  5. docs/roles/R1_SPINE.md  (my task list, in order)
  6. docs/02_SECURITY_AND_QUALITY.md  (all of it - it is mine)

I OWN and may edit ONLY these paths:
  apps/web/src/lib/*.ts      (db, auth, guard, money, http, env, audit, idem, ml)
...
docs/BLOCKERS.md, write a local stub or a TODO(L1): comment in my own file,
and keep moving. Never blocked longer than fifteen minutes.

Hard rules for you specifically:
- Scaffold apps/web FIRST and push it. Three other lanes are blocked until it lands.
...
(feat(spine): ...) and push. Commit every 30-45 minutes.

Start with T1.1 in docs/roles/R1_SPINE.md
and tell me your plan before you write any files.
```


## L2  ORACLE  -  suggested: Akash


**Mission.** A measured MASE of 0.91 with an honest caveat beats a claimed 95% accuracy every time. Your NO_ADVICE path is a feature, not a fallback.


**Order of work.** Python 3.11 environment, then real data on disk, then a baseline, then lightgbm, then the optimiser, then the pledge quote.


**Your brief.** `docs/roles/R2_ORACLE.md`
...
```PASTE THIS INTO CLAUDE CODE, EVERY SESSION
I am L2 (codename ORACLE) on MANDI-SETU, a Next.js 15 + Prisma + Postgres
monorepo for Smart India Hackathon 2026, problem statement 26132 (market
linkages and price discovery for farmers of Maharashtra). SIX engineers with
six Claude Code agents are building this in ONE repo over three days.
...
Read these before you write anything, in this order:
  ...
  3. docs/01_CONTRACTS.md             (frozen API surface)
  4. docs/03_DEMO_AND_SEED.md         (the numbers on screen are contractual)
  5. docs/roles/R2_ORACLE.md  (my task list, in order)
  6. prisma/schema.prisma and packages/contracts/src/index.ts  (read-only for me)

I OWN and may edit ONLY these paths:
  services/ml/**                          (the entire Python service)
...
If a change is needed anywhere else, do NOT edit it. Append an entry to
docs/BLOCKERS.md, write a local stub or a TODO(L2): comment in my own file,
and keep moving. Never blocked longer than fifteen minutes.

Hard rules for you specifically:
- Python 3.11 via `uv venv --python 3.11`. NOT python3 - default 3.14 has no LightGBM wheel.
...
After each task run `npm run typecheck && npm run test`, then commit
(feat(oracle): ...) and push. Commit every 30-45 minutes.

Start with T2.1 in docs/roles/R2_ORACLE.md
and tell me your plan before you write any files.
```


## L3  LEDGER  -  suggested: Nikhil


**Mission.** You own the number on the winning slide. It must be measured by subtraction, never asserted, never hardcoded.


**Order of work.** Escrow.ts and its tests before any route, then lots, then grading and split, then matching, offers and the ledger.


**Your brief.** `docs/roles/R3_LEDGER.md`
...
```PASTE THIS INTO CLAUDE CODE, EVERY SESSION
I am L3 (codename LEDGER) on MANDI-SETU, a Next.js 15 + Prisma + Postgres
monorepo for Smart India Hackathon 2026, problem statement 26132 (market
linkages and price discovery for farmers of Maharashtra). SIX engineers with
six Claude Code agents are building this in ONE repo over three days.
...
Read these before you write anything, in this order:
  ...
  3. docs/01_CONTRACTS.md             (frozen API surface)
  4. docs/03_DEMO_AND_SEED.md         (the numbers on screen are contractual)
  5. docs/roles/R3_LEDGER.md  (my task list, in order)
  6. docs/02_SECURITY_AND_QUALITY.md  (S1, S3, S9, S10, S11, S12 are mine)
  7. prisma/schema.prisma and packages/contracts/src/index.ts  (read-only for me)

I OWN and may edit ONLY these paths:
  apps/web/src/lib/domain/{grading,matching,escrow,ledger,split}.ts
...
If a change is needed anywhere else, do NOT edit it. Append an entry to
docs/BLOCKERS.md, write a local stub or a TODO(L3): comment in my own file,
and keep moving. Never blocked longer than fifteen minutes.

Hard rules for you specifically:
- escrow.ts FIRST, before any route, with its tests in the same commit.
...
Start with T3.1 in docs/roles/R3_LEDGER.md
and tell me your plan before you write any files.
```


## L4  KISAN  -  suggested: Shreya


**Mission.** The judge looks at your screens for the entire seven minutes. Marathi first, thumb-reachable, and a farmer must never see a stack trace.


**Order of work.** I18n before any screen, then the shell and login, then prices, then the window screen, then the refusal screen.


**Your brief.** `docs/roles/R4_KISAN.md`
...
```PASTE THIS INTO CLAUDE CODE, EVERY SESSION
I am L4 (codename KISAN) on MANDI-SETU, a Next.js 15 + Prisma + Postgres
monorepo for Smart India Hackathon 2026, problem statement 26132 (market
linkages and price discovery for farmers of Maharashtra). SIX engineers with
six Claude Code agents are building this in ONE repo over three days.
...
Read these before you write anything, in this order:
  ...
  3. docs/01_CONTRACTS.md             (frozen API surface)
  4. docs/03_DEMO_AND_SEED.md         (the numbers on screen are contractual)
  5. docs/roles/R4_KISAN.md  (my task list, in order)
  6. packages/contracts/src/fixtures.ts  (my back end until Sunday - read it fully)
...
If a change is needed anywhere else, do NOT edit it. Append an entry to
docs/BLOCKERS.md, write a local stub or a TODO(L4): comment in my own file,
and keep moving. Never blocked longer than fifteen minutes.

Hard rules for you specifically:
- i18n FIRST, before any screen. Marathi is the default, English is the option.
...
After each task run `npm run typecheck && npm run test`, then commit
(feat(kisan): ...) and push. Commit every 30-45 minutes.

Start with T4.1 in docs/roles/R4_KISAN.md
and tell me your plan before you write any files.
```


## L5  BAZAAR  -  suggested: Kartik


**Mission.** You own the story. If the seed data is not believable, nothing else in the demo matters.


**Order of work.** Reference data and warehouses first, then the personas, then prices, then history, then verification and rehearsal.


**Your brief.** `docs/roles/R5_BAZAAR.md`
...
```PASTE THIS INTO CLAUDE CODE, EVERY SESSION
I am L5 (codename BAZAAR) on MANDI-SETU, a Next.js 15 + Prisma + Postgres
monorepo for Smart India Hackathon 2026, problem statement 26132 (market
linkages and price discovery for farmers of Maharashtra). SIX engineers with
six Claude Code agents are building this in ONE repo over three days.
...
Read these before you write anything, in this order:
  ...
  3. docs/01_CONTRACTS.md             (frozen API surface)
  4. docs/03_DEMO_AND_SEED.md         (the numbers on screen are contractual)
  5. docs/roles/R5_BAZAAR.md  (my task list, in order)
  6. prisma/seed/rng.ts  (already written - every random number comes from here)

I OWN and may edit ONLY these paths:
  prisma/seed/**   EXCEPT 00_reference.ts (L1) and 10_prices.ts (L2)
  tests/e2e/**
  docs/03_DEMO_AND_SEED.md      docs/deck/SOURCES.md
  docs/roles/R5_BAZAAR.md
If a change is needed anywhere else, do NOT edit it. Append an entry to
docs/BLOCKERS.md, write a local stub or a TODO(L5): comment in my own file,
and keep moving. Never blocked longer than fifteen minutes.

Hard rules for you specifically:
- Everything deterministic. rngFor(name) only. No Math.random() anywhere under
...
- No Aadhaar numbers anywhere, not even fake ones. Phone is the identifier.
After each task run `npm run typecheck && npm run test`, then commit
(feat(bazaar): ...) and push. Commit every 30-45 minutes.

Start with T5.3 in docs/roles/R5_BAZAAR.md (T5.1 and T5.2 are already done)
and tell me your plan before you write any files.
```


## L6  CONSOLES  -  suggested: Nilesh


**Mission.** The consoles prove this is a two-sided market and not a farmer app with a story. The deck is what the college round actually judges.


**Order of work.** Buyer console shell and demand form, then matched lots with the why sentence, then fpo, then admin, and the deck from hour zero.


**Your brief.** `docs/roles/R6_CONSOLES.md (write it from this section first)`
...
```PASTE THIS INTO CLAUDE CODE, EVERY SESSION
I am L6 (codename CONSOLES) on MANDI-SETU, a Next.js 15 + Prisma + Postgres
monorepo for Smart India Hackathon 2026, problem statement 26132 (market
linkages and price discovery for farmers of Maharashtra). SIX engineers with
six Claude Code agents are building this in ONE repo over three days.
...
Read these before you write anything, in this order:
  ...
  3. docs/01_CONTRACTS.md             (frozen API surface)
  4. docs/03_DEMO_AND_SEED.md         (the numbers on screen are contractual)
  5. docs/roles/R5_BAZAAR.md sections T5.7, T5.8, T5.9 and T5.13  (my tasks, carved out of R5)
  6. packages/contracts/src/fixtures.ts  (my back end until Sunday)
...
If a change is needed anywhere else, do NOT edit it. Append an entry to
docs/BLOCKERS.md, write a local stub or a TODO(L6): comment in my own file,
and keep moving. Never blocked longer than fifteen minutes.

Hard rules for you specifically:
- My first commit is docs/roles/R6_CONSOLES.md, written from T5.7, T5.8, T5.9 and
...
After each task run `npm run typecheck && npm run test`, then commit
(feat(consoles): ...) and push. Commit every 30-45 minutes.

Start with writing docs/roles/R6_CONSOLES.md, then T5.7
and tell me your plan before you write any files.
```
...
You will run out of time. Every team does, and the teams that lose are the ones deciding what to drop at 02:00 on the last night, tired, with four people arguing. Decide now, while it is cheap, and write it on the whiteboard.


## Drop in this order, and only in this order


| # | Drop | Costs you | Say this instead |
|---|---|---|---|
...
## Never drop these, under any circumstance
...
> **The cut list is Lane 1's to invoke, at Sunday 20:00, out loud.**  
...
# 08. The Protocol for Six Agents in One Repository


The largest risk to this project is not missing features. It is two agents editing the same file and destroying each other's work at merge time. Everything below exists for that reason.


## Sync windows - ten minutes, and non-negotiable


Friday 19:00, Saturday 12:00, Saturday 23:00, Sunday 12:00, Sunday 20:00, Monday 07:00. Round-robin, ninety seconds each, exactly three sentences: what merged since last sync, what I am on now, and what I need from whom - and that had better already be in `BLOCKERS.md`. Then Lane 1 merges in dependency order - 1, then 2 and 3, then 4, 5 and 6 - and announces "main is at X, rebase now." Everybody rebases before writing another line.


## The blocker protocol - fifteen minutes, maximum


You will need something you do not own: a schema field, a contract shape, an endpoint that does not exist. Waiting stalls you. Editing their file collides. The right move, every single time:


1. **Append** to `docs/BLOCKERS.md`. Never edit or delete another entry.
2. **Stub locally** so you keep moving - a hardcoded object, a fixture from `@mandi/contracts/fixtures`, or a `TODO(L3):` comment naming the exact shape you assumed.
3. **Say it out loud** at the next sync. The file is the record; speech is the alert. Both, always.
4. When it lands, delete your stub. `grep -rn "TODO(L"` on Sunday is your punch list.


The invariant: **no agent is blocked for more than fifteen minutes.** If you are, you are doing it wrong. Stub and move.


```THE FORMAT. COPY IT EXACTLY.
### [L4 -> L1] CONTRACT: WindowRes needs worstCaseTotalPaise
...
## The ten invariants, condensed


These are not style preferences. Breaking one is a bug even if the tests pass. The full text is in `CLAUDE.md` and it is binding.


| # | Invariant | Why it is a rule and not a suggestion |
|---|---|---|
...
## Definition of done - all eight, not six of eight


1. `npm run verify` passes: typecheck, lint, unit tests, production build.
2. It works against **seeded** data with no external network call.
3. Zod validation on every input. Bad input returns a coded 400, not a 500.
4. Another actor's id returns 404, never their data. You have tested this by hand with two cookies and `curl`, not by reading the code.
5. Money is paise the whole way. You have grepped your own diff for `parseFloat`, `Number(` and `.toFixed`.
6. Empty state, loading state and error state all render. Not just the happy path.
7. Marathi strings exist for anything a farmer sees. Request them from Lane 4 if missing.
...
# 09. What Would Lose This For Us


Read this list once a day. Every item is a specific thing a team like ours has actually done.


| Failure | How it happens | The counter-move |
|---|---|---|
...
# 10. The Next Sixty Minutes


Not the next three days. The next hour, per person, starting when you finish reading this.


| Who | In the next hour |
|---|---|
...
The internal submission shows 80-90%. Everything below is what closes the gap before the main SIH hackathon, and naming it honestly on a roadmap slide reads as maturity. Claiming you built it reads as a lie the moment a judge clicks.


| Phase 2 | Why it was out of scope for three days |
|---|---|
...
# 12. Pushing the Repository


Everything currently in the working tree is committed locally on `main`. The push could not be completed from this session because outbound network access to GitHub is blocked in the sandbox. Run this yourself, once, from the repository root:
...
If it asks for credentials, use a GitHub personal access token as the password, not your account password. Then confirm all six people can clone and push before Friday 19:00 - a permissions problem discovered on Sunday costs a whole evening.


## What is in that commit


Commit `11151d9`, on top of `a4e1693`: the contracts package, the Prisma schema, eleven documents, five role briefs, the seed skeleton and its deterministic PRNG, the ML contract mirror, and the workspace configuration. Only `.env.example` is committed - no secrets, per I7. Nine stale `.pyc` files were removed from tracking and `.gitignore` was extended to keep them out.


## One thing to check before anyone else clones


Confirm the repository is private if you intend it to be. And confirm that `.env` is absent from `git ls-files` on every machine, not just yours - the file is gitignored, but a teammate who copies it under a different name defeats that in one command.


> **The one sentence to keep in front of you for three days**  
