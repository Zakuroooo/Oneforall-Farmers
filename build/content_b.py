# -*- coding: utf-8 -*-
"""Chapters 02-03: root-cause analysis, existing landscape and gap."""

def build(d, PDF=False):
    d.h1('Root Cause: Why the Farmer Loses the Money','02')
    d.p('If you get one chapter right for the pitch, make it this one. Judges have heard "farmers face problems" a hundred times. What wins is a precise causal model with named mechanisms, because a precise model implies a precise intervention, and a precise intervention is a product.')
    d.p('The gap between what a consumer pays and what a farmer receives is not one leak. It is five, and they have different physics. Confusing them is why most agri-tech fails: it builds one tool and expects it to close five gaps.')

    d.h2('2.1 The five loss mechanisms')
    d.h3('Mechanism 1: Information asymmetry (the price the farmer does not know)')
    d.p('The farmer knows one price: the one shouted in their own mandi this morning. The trader knows several: this mandi, the next district, the terminal market in Mumbai or Delhi, and the direction prices are heading this week. The gap between those two knowledge sets is captured entirely by the party with more information.')
    d.p('This is not a hypothesis. The problem statement itself opens with it: "limited visibility of current and expected prices across nearby markets". The official diagnosis and ours agree.')
    d.p('Note carefully what asymmetry means, because it changes the product. It is not that price data does not exist - Agmarknet publishes it. It is that the data is (a) published after the fact, (b) as a modal or min/max range that hides the distribution, (c) in a form and language and interface a smallholder does not consume, and (d) with no forward view. Publishing more of the same data does not close an asymmetry gap. Converting data into a decision does.')
    d.callout('PRODUCT CONSEQUENCE',
      'The unit of value is not a price, it is a recommendation with a confidence interval and a reason. '
      '"Onion at Lasalgaon is Rs 1,240/qtl" is data. "Hold 12 days: Lasalgaon is forecast to rise to Rs 1,390 with 70% confidence, '
      'your storage cost is Rs 62, expected gain Rs 88/qtl net, but if you need cash now, pledge at 9% and still net Rs 61" is a product.')

    d.h3('Mechanism 2: Liquidity coercion (the distress sale)')
    d.p('This is the most important mechanism and the least served by existing software. A farmer harvests. They owe money now - to an input dealer, to a moneylender, for a wedding, for school fees, for a medical bill. They have no working capital and often no storage. So they sell on day one at whatever the market gives, even when they know prices will rise in three weeks.')
    d.p('The problem statement names it directly: farmers "may sell immediately after harvest because of liquidity or storage constraints".')
    d.p('Now look at the credit structure that makes this a trap. Debt is the most consistently cited factor in farmer suicides across the literature, and a 2014 study found that three traits together - cash crops such as cotton, farms under one hectare, and existing debt - explained "almost 75% of the variability in state-level suicides" (Wikipedia, Farmers\' suicides in India). Non-institutional interest rates have been documented as high as 21 percent (same source, Punjab data). In Maharashtra, 43 percent of farmers are marginal, holding under one hectare, and average holdings across all size groups are below three hectares (Wikipedia, Agriculture in Maharashtra).')
    d.p('So the typical Maharashtra farmer is precisely the profile the suicide literature identifies as highest risk: small holding, cash crop, indebted. And the mechanism by which a bad price becomes a death is liquidity: the farmer cannot wait, so they take the bad price, so the debt does not clear, so they borrow again at a worse rate.')
    d.callout('PRODUCT CONSEQUENCE - AND THE HEART OF OUR SOLUTION',
      'Advice without liquidity is cruelty. Telling a farmer "prices will rise, hold your crop" when they must repay a moneylender '
      'on Friday is worse than useless - it converts a financial problem into a psychological one. Therefore: every HOLD recommendation '
      'our system makes must come bundled with a cash option. That option is pledge finance against an electronic negotiable warehouse '
      'receipt. The crop goes into a registered warehouse, an e-NWR is issued, a lender advances 70-75 percent of value against that receipt, '
      'the farmer gets cash today AND keeps the upside when they sell later. This one loop is the difference between an advisory app and an '
      'intervention. Say this sentence in the pitch: "We do not tell farmers to wait. We pay them to wait."',
      color=(0.10,0.45,0.30), bg=(0.94,0.98,0.95))

    d.h3('Mechanism 3: Fragmentation (weak bargaining power)')
    d.p('A single smallholder arriving with 8 quintals is a price-taker. A licensed trader in a mandi with a handful of peers is a price-setter. The Wikipedia article on APMCs states it flatly: "cartelization has been a big problem in APMCs", and the central premise of the 2020 reform attempt was the "monopoly of middlemen".')
    d.p('The structural detail that matters: only licensed traders may operate inside a mandi, and wholesalers, retailers and processors historically could not buy directly from a farmer (Wikipedia, APMC). That is a legally enforced bottleneck. Even where Maharashtra has relaxed it for fruits and vegetables, the software to route around the bottleneck does not exist for a farmer with a feature phone.')
    d.p('Aggregation is the known answer, and it is the reason the Government of India built the 10,000 FPO programme. But FPOs have their own failure mode: an FPO needs to convince 60 farmers that a pooled lot will be split fairly, and without a transparent split ledger, trust collapses at the first disagreement about whose onions were the good ones.')
    d.callout('PRODUCT CONSEQUENCE',
      'Do not just "support FPOs". Build the thing FPOs actually lack: a grade-weighted fair-split ledger. Each farmer\'s contribution '
      'to a pooled lot is recorded with its own grade assay, the lot sells as one, and the proceeds split proportional to '
      'quantity times grade multiplier, visible to every contributing farmer on their phone before they agree to pool. '
      'Fairness must be verifiable in advance, not explained afterwards.')

    d.h3('Mechanism 4: Physical loss (post-harvest and first-mile)')
    d.p('Produce rots. It rots in the field waiting for a truck, in the truck waiting at the mandi gate, and in the mandi waiting for a bid. Each hour is a percentage of value. The Wikipedia article on post-harvest losses is unusually candid that reliable aggregate loss numbers do not exist - "there are no reliable methods for evaluating post-harvest losses of fresh produce" - and warns that averaged figures mislead because real outcomes range from near-zero to total write-off when "a price collapse makes harvesting uneconomic".')
    d.p('That last clause is the important one and it is a lesson for the pitch: some post-harvest loss is not a cold-chain problem, it is a price-information problem. A farmer who leaves tomatoes unharvested because the price does not cover the picking cost has suffered a 100 percent loss caused by market failure, not by refrigeration failure. Our system attacks that specific loss by finding a buyer at a price that makes harvest worthwhile, including lower-grade buyers such as processors who will take produce that the fresh market rejects.')
    d.callout('PRODUCT CONSEQUENCE',
      'Grade-differentiated routing. A lot that fails fresh-market grade is not waste - it is processor feedstock at a lower but positive price. '
      'Most platforms treat quality as a binary pass/fail. We treat it as a routing key: Grade A to retail and export, Grade B to wholesale, '
      'Grade C to processing, Grade D to feed or dehydration. Every kilogram finds its highest-value home rather than its only home. '
      'This is also, incidentally, exactly what the problem statement means by "reduced post harvest loss".')

    d.h3('Mechanism 5: Counterparty risk (the money that never arrives)')
    d.p('A price agreed is not a price received. Payment delays, partial payments, post-facto quality disputes used as a lever to renegotiate downward, and outright default are ordinary experiences. The problem statement names it: "payment reliability and buyer credentials may be fragmented".')
    d.p('Note the second-order effect, which is the subtle part. Because payment risk exists, farmers rationally prefer a known local trader who pays cash at a lower price over an unknown distant buyer offering a higher price. Counterparty risk is therefore not just a loss in itself - it is a tax on every other improvement you make. You can show a farmer a better price 200 km away and they will still sell locally, correctly, because the better price is not credible.')
    d.callout('PRODUCT CONSEQUENCE',
      'Trust must be engineered before better prices can be delivered. Three layers: (1) verification tiers for buyers, '
      'with GSTIN, APMC licence, PAN and bank-account verification, displayed as a badge; (2) a payment-reliability score '
      'computed from actual settled transactions on-platform - days-to-pay distribution, dispute rate, renegotiation rate; '
      '(3) escrow or assured-payment rails for high-value lots so the farmer\'s downside is bounded. '
      'Without these, higher prices displayed are higher prices ignored.')

    d.h2('2.2 The causal chain, stated plainly')
    d.p('For the pitch, compress the five mechanisms into one causal chain. This is your opening 60 seconds.')
    d.code(
...
'                    |\n'
'                    v\n'
'          the outcome this problem statement exists to prevent',label='causal-chain')
    d.p('The loop is the insight. This is not a static problem, it is a ratchet - each bad season reduces the farmer\'s ability to survive the next one. Which means an intervention does not need to fix the whole gap in one go. It needs to break the ratchet, by giving the farmer the ability to wait even once. A farmer who can wait one season enters the next season with less debt and more ability to wait. The system compounds in the farmer\'s favour instead of against them.')
    d.callout('THE STRATEGIC CLAIM THAT WINS THIS PROBLEM STATEMENT',
      'Our thesis in one sentence: the binding constraint on farmer price realisation is not information, it is the ability to wait. '
      'Everyone else at this hackathon will build an information product. We are building a waiting product - forecast, storage, '
      'and pledge credit as one loop - and information is merely the input that makes the waiting decision correct. '
      'That is the difference between a price dashboard and a price outcome.',
      color=(0.10,0.45,0.30), bg=(0.94,0.98,0.95))

    d.h2('2.3 Quantifying the gap: what to say and what not to say')
    d.p('You will be tempted to open with a big number: "farmers get only 30 percent of the consumer rupee". Be careful. Judges from the agriculture department know these numbers are contested and vary hugely by crop and channel. A number you cannot defend is worse than no number.')
    d.p('Here is how to handle it. Use verified structural facts as your foundation, and generate your own numbers from the demo rather than borrowing shaky ones.')
    d.table(['Claim type','Example','Verdict'],[
    ...
    d.callout('DO THIS BEFORE THE PITCH',
      'Build the Realisation Ledger early and run it over historical Agmarknet data as a backtest. Then your headline number is not '
      'a borrowed statistic, it is your own measured result: "on three years of historical Maharashtra onion data, our sale-window engine '
      'would have improved median realisation by Rs X per quintal against a sell-on-harvest-day baseline, after storage and finance costs." '
      'A backtested number that you computed beats any citation, and it is the single highest-leverage piece of pre-work in this document.')

    # ============ CH 3 ============
    d.h1('The Existing Landscape and the Honest Gap','03')
    d.p('Judges will ask: eNAM exists, Agmarknet exists, there are a dozen agri-tech startups. Why you? If you cannot answer this crisply you lose, regardless of how good your build is. This chapter is your ammunition. The rule is: be scrupulously fair to what exists, then be precise about the gap.')

    d.h2('3.1 Government infrastructure')
    d.table(['System','What it does','Verified scale','The gap we exploit'],[
    ...
    d.h2('3.2 The literature: what researchers have already proven')
    d.p('This section exists so you can say "the research says X, so we did Y" instead of "we thought Y would be nice". Every one of these was retrieved from arXiv during research for this playbook. Read at least the four starred ones.')
    d.table(['Paper','ID','Finding that changes our design'],[
    ...
    d.h2('3.3 Private sector: who is already doing what')
    d.p('Be honest and specific here. Naming competitors accurately signals maturity; pretending you have no competition signals the opposite.')
    d.table(['Player type','Examples','What they do well','Structural gap'],[
    ...
    d.h2('3.4 The gap, stated in one paragraph you should memorise')
    d.callout('THE POSITIONING STATEMENT',
      'Everything that exists is either a price PUBLISHER without a decision layer (Agmarknet), a TRADING VENUE that still assumes '
      'the farmer comes to the mandi and mostly trades within it (eNAM), a BUYER pretending to be a platform (B2B marketplaces), '
      'or a LENDER with a platform attached (agri fintech). Nobody has built the neutral layer that combines a forecast the farmer can act on, '
      'the credit that lets them act on it, the aggregation that gives them bargaining power, the grading that makes their lot tradable at distance, '
      'and the settlement guarantee that makes a distant price credible - and then MEASURES whether the farmer actually earned more. '
      'That combination is the product. Not one of those five pieces is novel alone. The loop is the invention.',
      color=(0.10,0.45,0.30), bg=(0.94,0.98,0.95))
    d.h3('Say this when a judge says "eNAM already exists"')
    d.p('"eNAM is a venue. We are a decision layer, and we integrate with eNAM rather than replacing it. eNAM answers: how do I trade in this mandi electronically. We answer three questions eNAM does not: should I sell today or in 12 days, how do I get cash if I choose to wait, and which of these buyers will actually pay me. By eNAM\'s own documentation the trade on it is mostly intra-market, so a farmer in Yavatmal still cannot reach a better price in Nagpur. That is the gap we close, and where eNAM has an API we push our lots into it as an additional demand channel."')
    return d
